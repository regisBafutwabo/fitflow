...
const WorkoutTracker: React.FC<WorkoutTrackerProps> = ({ userId }) => {
  ...
  const [createWorkoutLog,{loading:createLogLoading}] = useCreateWorkoutLogMutation();
  const [createExerciseLog] = useCreateExerciseLogMutation();

  useEffect(() => {
    if (data?.workout_plans[0]) {
      const plan = data.workout_plans[0];
      setWorkouts(plan.workout_days.map(day => ({
        ...day,
        date: '',
  ...
  const handleDateChange = (index: number, date: string) => {
    ...
    newWorkouts[index].exercises = newWorkouts[index].exercises.map((exercise:any) => {
        const loggedExercise = exercise?.logs?.filter((log:any) => log?.workout_log?.date===date)[0];
        return{...exercise,
  ...
  const handleWeightChange = (dayIndex: number, exerciseIndex: number, weight: string) => {
    const newWorkouts = [...workouts];
    newWorkouts[dayIndex].exercises[exerciseIndex].weight = weight;
    setWorkouts(newWorkouts);
  };

  const handleCompletedChange = async (dayIndex: number, exerciseIndex: number) => {
    const newWorkouts = [...workouts];
      ...
      try {
        const { data: workoutLogData } = await createWorkoutLog({
          variables: {
            ...
            date: newWorkouts[dayIndex].date || new Date().toISOString().split('T')[0],
          },
        });
  ...
  const completedExercises = workouts.flatMap(day => day.exercises.filter((ex:Exercise_Logs) => ex.completed)).length;

  return (
    <div className="bg-white shadow-md rounded px-4 sm:px-8 pt-6 pb-8 mb-4">
      <h1 className="text-xl sm:text-2xl font-bold mb-4 text-center">6-Day Machine-Based Workout Plan for Beginners</h1>
      <div className="mb-4">
        <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="week-starting">
          Week Starting:
        </label>
        <input
      ...
      {workouts.map((day, dayIndex) => (
        <div key={day.day} className="mb-6 bg-gray-50 rounded-lg p-4">
          <h2 className="text-lg sm:text-xl font-semibold mb-2">{day.day_of_week} - {day.muscle_group}</h2>
          <input
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {day.exercises.map((exercise: { name: boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | Promise<React.AwaitedReactNode> | React.Key | null | undefined; weight: string | number | readonly string[] | undefined; completed: boolean | undefined; }, exerciseIndex: any) => (
                  <tr key={exercise?.name as string} className="border-b">
                    <td className="px-2 sm:px-4 py-2">{exercise.name}</td>
                    <td className="px-2 sm:px-4 py-2">{exercise.name === 'Stairmaster' ? '20 mins at speed 4' : '3 x 12'}</td>
                    <td className="px-2 sm:px-4 py-2">
                      <input
                        ...
                        onChange={(e) => handleWeightChange(dayIndex, exerciseIndex, e.target.value)}
                        disabled={exercise.name === 'Stairmaster' || day.day === 'Sunday'}
                      />
                    </td>
                    <td className="px-2 sm:px-4 py-2">
                      <input
                        ...
                        onChange={() => handleCompletedChange(dayIndex, exerciseIndex)}
